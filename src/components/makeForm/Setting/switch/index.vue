<template>
  <item-box classSuffix="switch">
    ...
    <el-form-item label="默认值">
      ...
      <el-select v-model="value" placeholder="请选择默认值">
    ...
    <el-form-item label="打开图标">
      <el-input v-model="data.setting.activeIconClass">
        <el-button slot="append" @click="openSelectIcon('activeIconClass')">选择图标</el-button>
      </el-input>
    </el-form-item>
    <el-form-item label="关闭图标">
      <el-input v-model="data.setting.inactiveIconClass">
        <el-button slot="append" @click="openSelectIcon('inactiveIconClass')">选择图标</el-button>
      </el-input>
    </el-form-item>
...
export default {
  ...
  methods: {
    ...
    toBoolean(x) {
      if (!x || x === '0' || x === 'false' || x === 'null' || x === 'undefined' || x === 'NaN') {
        return false
      } else return true
    },
    setValue(v, watch) {
    ...
    toUtilValue(type) {
      if (type === 'Boolean') {
        ...
        this.data.setting.inactiveValue = this.toBoolean(this.inactiveValue)
      } else if (type === 'Number') {
