<template>
  <item-box classSuffix="time-picker-multiple">
    ...
    <el-form-item label="范围">
      <el-input v-model="selectableRange" placeholder="如：00:00:00-23:59:59,00:00:00-23:59:59"></el-input>
    </el-form-item>
    ...
    <el-form-item label="前图标">
      <el-input v-model="data.setting.prefixIcon">
        <el-button slot="append" @click="openSelectIcon('prefixIcon')">选择图标</el-button>
      </el-input>
    </el-form-item>
...
export default {
  ...
  created() {
    this.selectableRange = `${this.data.setting.selectableRange[0]},${this.data.setting.selectableRange[1]}`;
  },
  watch: {
    defaultValue(v) {
      if (v.includes(",")) {
        const y = v.split(",");
        if (y.length >= 2 && /^(([0-1][0-9])|(2[0-3])):[0-9][0-9]:[0-9][0-9]$/.test(y[0]) && /^(([0-1][0-9])|(2[0-3])):[0-9][0-9]:[0-9][0-9]$/.test(y[1])) {
          const t0 = new Date();
          const x0 = y[0].split(":");
